#!/usr/bin/python3
"""
Fabric script that creates and distributes an archive to web servers using the deploy function
"""
...
def do_deploy(c, archive_path):
    ...
    try:
        ...
        c.run('tar -xzf /tmp/{} -C /data/web_static/releases/{}/'.format(archive_filename, archive_name))

        # Delete the archive from the web server
        c.run('rm /tmp/{}'.format(archive_filename))

        # Move the extracted contents to the parent directory
        c.run('mv /data/web_static/releases/{}/web_static/* /data/web_static/releases/{}/'.format(archive_name, archive_name))

        # Remove the empty web_static directory
        c.run('rm -rf /data/web_static/releases/{}/web_static'.format(archive_name))

        # Delete the symbolic link /data/web_static/current
        c.run('rm -rf /data/web_static/current')

        # Create a new symbolic link to the new version of the code
        c.run('ln -s /data/web_static/releases/{}/ /data/web_static/current'.format(archive_name))
